...
- PostHog is initialized by the production simulator entry point; successful simulations emit `battle_run`. `simulationAnalytics.ts` adds `job_id`, `status`, requested/completed simulation counts, server-derived `queue_wait_ms`, `execution_ms`, `total_job_ms`, `execution_timeout_ms`, and `execution_ms_per_simulation`. Failed, timed-out, cancelled, and expired jobs emit `battle_run_unsuccessful` when observed by the browser; count distinct `job_id` values because reconnecting to these terminal jobs can report them again. Unstarted execution and unavailable completed counts/per-simulation timings are null. Events exclude army contents, logs, and error text. Analytics failures do not block results or acknowledgement. Lifecycle events are `battle_submission_started`, `battle_submission_accepted`, `battle_submission_failed`, `battle_recovery_started`, `battle_cancel_requested`, and `battle_cancel_failed`. Submission events count attempts (including explicit retries/recovery), correlate through `job_id`, and include the captured requested count; acceptance/failure includes browser-monotonic `submission_ms`. Failure reasons are bounded categories (`network`, `queue_full`, `http_error`, `invalid_response`) with HTTP status when available. Recovery includes whether the job was already accepted; cancellation events describe the request, with the eventual outcome reported separately. These are browser-observed outcomes, not a complete backend job census (closed tabs and legacy `/battle` clients are not tracked).
...
Simulation jobs use the same SQLite database in a separate `simulation_jobs` table. Run **one backend instance per database**: the scheduler owns up to two engine execution slots and is not a distributed queue. Keep the database on persistent storage when deploying a container. Defaults are a five-minute execution deadline (including a two-second termination grace period), ten-minute queue expiry, two concurrent simulations, 20 waiting jobs, and one-minute retention of unacknowledged results and other terminal jobs. Completed results are deleted immediately after client acknowledgement. Server environment overrides are `SIMULATION_TIMEOUT_MS`, `SIMULATION_QUEUE_TIMEOUT_MS`, `SIMULATION_MAX_QUEUED`, and `SIMULATION_RETENTION_MS`; positive integers are required, with maxima of one hour, 24 hours, 1000 waiting jobs, and seven days respectively. `SIMULATION_MAX_CONCURRENT` can lower concurrency to 1; it defaults to 2 and rejects values above 2. The grace period is shortened for execution limits below 20 seconds. Pending jobs survive restarts; jobs interrupted during execution are marked failed, not automatically rerun. SIGTERM/SIGINT stop scheduling and terminate all active engines before closing SQLite.
