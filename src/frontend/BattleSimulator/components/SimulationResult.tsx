    ...
    return (
        <>
            <Tabs
                ...
                aria-label="Example battle outcomes"
                variant="scrollable"
                scrollButtons="auto"
            >
                {logs.map((log) => (
                    <Tab
                        ...
                        value={log.label}
                        id={`${id}-${log.label}-tab`}
                        aria-controls={`${id}-${log.label}-panel`}
                    />
                ))}
            </Tabs>
            <Box
                role="tabpanel"
                id={`${id}-${active.label}-panel`}
                aria-labelledby={`${id}-${active.label}-tab`}
                tabIndex={0}
                sx={{mt: 2}}
            >
                <Box
...
export const SimulationResult: FC<ServerSimulationResponse> = ({
    ...
    const casualties = [
        {
            label: 'Average per battle',
            attacker: realNumber(attackerLooses.mean),
            defender: realNumber(defenderLooses.mean),
        },
        ...
        <StyledPaper elevation={3} sx={{mt: 2, p: {xs: 2, sm: 3}}}>
            ...
            >
                ...
                <Typography variant="body2" color="text.secondary">
                    {runs} {runs === 1 ? 'simulation' : 'simulations'}
                </Typography>
            </Box>
            <Box sx={{my: 3}}>
                ...
                <LinearProgress
                    variant="determinate"
                    value={Math.max(0, Math.min(100, displayedRatio))}
                    aria-label="Attacker win rate"
            ...
            <TableContainer sx={{mb: 3}}>
                <Table
                    size="small"
                    aria-label="Casualty comparison"
