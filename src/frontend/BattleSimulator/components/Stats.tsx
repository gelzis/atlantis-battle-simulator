...
export default function Stats({runs, occurance, min, max, mean, median, mode, stdDev, percentile}: StatsProps) {
    ...
    return (
        <StatsContainer>
            <StatValue
                title="Occurrence"
                description="Share of simulations with casualties on this side."
                value={percent(runs > 0 ? occurance / runs : 0)}
            />
            <StatValue
                title="Expected"
                description="Mean plus or minus one standard deviation, rounded down and bounded at zero. This is a descriptive range, not a guarantee."
                value={expected}
            />
            ...
            <StatValue
                title="Median"
                description="Middle casualty count when simulation results are sorted."
                value={realNumber(median)}
            />
            <StatValue title="Mode" description="Most frequent casualty count." value={realNumber(mode)} />
            <StatValue
                title="Std. dev."
                description="How much casualty counts vary around the average."
                value={realNumber(stdDev)}
            />
            <StatValue
                title="Percentiles"
                description="Casualty thresholds across the distribution. Hover over a bar for its percentile and value."
                value={<PercentileGraph items={percentile} />}
            />
        </StatsContainer>
    );
}
