...
export function LocalPersistence({
...
}: PropsWithChildren<{storage?: () => StoragePort}>) {
    ...
    useEffect(() => {
        ...
        let status = enabled
            ? draft.value
                ? 'Draft restored. Changes save in this browser.'
                : 'Changes save automatically in this browser.'
            : 'Shared battle: your local draft is kept untouched.';

        const publish = () =>
            setSession({
                baseline: baseline.value,
                warning: [
                    draft.warning && `Draft: ${draft.warning}`,
                    baseline.warning && `Baseline: ${baseline.warning}`,
                ]
                    .filter(Boolean)
                    .join(' '),
                status,
...
export function DraftStatus() {
    ...
    if (!session) return null;
    return (
        <Typography component="div" variant="caption" sx={{color: 'inherit', opacity: 0.8, lineHeight: 1.5}}>
...
export function BaselineComparison({
    ...
    const rows =
        baseline && current
            ? [
                  [
                      'Attacker win rate',
                      `${realNumber(baseline.result.winRate)}%`,
                      `${realNumber(current.result.winRate)}%`,
                      `${delta(baseline.result.winRate, current.result.winRate)} pp`,
                  ],
                  [
                      'Average attacker casualties',
                      realNumber(baseline.result.attackerMean),
                      realNumber(current.result.attackerMean),
                      delta(baseline.result.attackerMean, current.result.attackerMean),
                  ],
                  [
                      'Average defender casualties',
                      realNumber(baseline.result.defenderMean),
                      realNumber(current.result.defenderMean),
                      delta(baseline.result.defenderMean, current.result.defenderMean),
                  ],
                  [
                      'Simulations',
                      String(count(baseline)),
                      String(count(current)),
                      delta(count(baseline), count(current)),
                  ],
              ]
            : [];
    ...
    return (
        <>
            <Accordion
                expanded={expanded}
                onChange={(_event, open) => setExpanded(open)}
                disableGutters
                elevation={0}
                variant="outlined"
            ...
            >
                <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                    <Typography variant="body2" color="text.secondary">
                        {baseline ? 'Baseline comparison · Pinned' : 'Compare battle results with baseline'}
                    </Typography>
                </AccordionSummary>
                <AccordionDetails>
                    {baseline && (
                        <Typography variant="body2" color="text.secondary">
                            Baseline from {new Date(baseline.completedAt).toLocaleString()} · {count(baseline)}{' '}
                            simulations
                        </Typography>
                    )}
                    <Box sx={{display: 'flex', flexWrap: 'wrap', gap: 1, my: 2}}>
                        {current && (
                            <Button onClick={() => session.pin(current)}>
                                {baseline ? 'Replace baseline with this result' : 'Pin as baseline'}
                            </Button>
                        )}
                        {baseline && (
                    ...
                    {rows.length > 0 ? (
                        <>
                            <TableContainer>
                                <Table size="small" aria-label="Baseline comparison">
                                    <TableHead>
                                        <TableRow>
                                            {['Metric', 'Baseline', 'Current', 'Change'].map((label) => (
                            ...
                            <Typography variant="body2" color="text.secondary" sx={{mt: 2}}>
                                Results are estimates. Small differences may be random variation. Win-rate changes are
            ...
            <Dialog open={confirm} onClose={() => setConfirm(false)} aria-labelledby="restore-baseline-title">
                ...
                <DialogContent>
                    This replaces your current armies, unfinished unit edits, and simulation count.
                </DialogContent>
                <DialogActions>
