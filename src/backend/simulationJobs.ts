...
export class SimulationJobs {
    ...
    constructor(databasePath: string, private runner: EngineRunner = createEngineRunner(), private options = defaultJobOptions) {
    ...
    private run(sql: string, params: unknown[] = []): Promise<void> {
        return new Promise((resolve, reject) => this.database.run(sql, params, error => error ? reject(error) : resolve()));
    }

    private rows(sql: string, params: unknown[] = []): Promise<JobRow[]> {
        return new Promise((resolve, reject) => this.database.all(sql, params, (error, rows: JobRow[]) => error ? reject(error) : resolve(rows)));
    }
    ...
    async initialize(): Promise<void> {
        ...
        await this.run("UPDATE simulation_jobs SET status = 'failed', finishedAt = ?, error = ? WHERE status = 'running'",
    ...
    submit(id: string, battle: unknown, battleCount: number): Promise<SimulationJob> {
        return this.exclusive(async() => {
            ...
            if (queued.length >= this.options.maxQueued) throw new JobRequestError(429, 'The simulation queue is full. Please try again later.');
            await this.run(`INSERT INTO simulation_jobs (id, status, battleJson, battleCount, createdAt, executionTimeoutMs)
                VALUES (?, 'queued', ?, ?, ?, ?)`, [id, battleJson, battleCount, Date.now(), this.options.executionTimeoutMs]);
            await this.schedule();
            return this.get(id);
        });
    }
    ...
    cancel(id: string): Promise<SimulationJob> {
        return this.exclusive(async() => {
            ...
            if (job.status === 'queued') {
                await this.run("UPDATE simulation_jobs SET status = 'cancelled', finishedAt = ?, error = ? WHERE id = ?",
    ...
    tick(): Promise<void> { return this.exclusive(() => this.schedule()); }

    private async schedule(): Promise<void> {
        if (this.stopped) return;
        const now = Date.now();
        await this.run("UPDATE simulation_jobs SET status = 'expired', finishedAt = ?, error = ? WHERE status = 'queued' AND createdAt <= ?",
            [now, 'Simulation expired while waiting in the queue. Please run it again.', now - this.options.queueTimeoutMs]);
        await this.run('DELETE FROM simulation_jobs WHERE finishedAt <= ?', [now - this.options.retentionMs]);
        if (this.active) return;
        const next = (await this.rows("SELECT * FROM simulation_jobs WHERE status = 'queued' ORDER BY createdAt, rowid LIMIT 1"))[0];
        if (!next) return;
        ...
        const finished = Promise.resolve().then(() => this.runner(JSON.parse(next.battleJson), next.battleCount, controller.signal, next.executionTimeoutMs))
            .then(result => this.finish(next.id, 'completed', JSON.stringify(result), null), error => {
                const failure = error instanceof EngineFailure ? error : new EngineFailure('failed', 'Simulation could not be completed.');
                if (failure.status === 'failed') {
                    console.error('Simulation engine failure', {jobId: next.id, message: failure.message, diagnostics: failure.diagnostics || String(error)});
                }
    ...
    private finish(id: string, status: SimulationJob['status'], result: string | null, error: string | null): Promise<void> {
        return this.exclusive(async() => {
            ...
            await this.run('UPDATE simulation_jobs SET status = ?, finishedAt = ?, resultJson = ?, error = ? WHERE id = ?',
