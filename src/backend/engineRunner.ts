...
export class EngineFailure extends Error {
    constructor(public status: 'failed' | 'timed_out' | 'cancelled', message: string, public diagnostics?: EngineDiagnostics) {
        super(message);
    }
}

export type EngineRunner = (battle: unknown, count: number, signal: AbortSignal, timeoutMs: number) => Promise<ServerSimulationResponse>;
...
export const createEngineRunner = (
...
): EngineRunner => async(battle, count, signal, timeoutMs) => {
    ...
    try {
        ...
        if (Date.now() >= deadline) throw new EngineFailure('timed_out', 'Simulation exceeded its execution time limit.');
        return await new Promise<ServerSimulationResponse>((resolve, reject) => {
            const child = spawn(executable, [...prefixArgs, 'battle', file, String(count)], {stdio: ['ignore', 'pipe', 'pipe']});
            const output: Buffer[] = [];
            ...
            const terminate = (reason: EngineFailure) => {
                ...
                failure = reason;
                child.kill('SIGTERM');
                cancelTimer = setTimeout(() => child.kill('SIGKILL'), Math.min(graceMs, Math.max(0, deadline - Date.now())));
            };
            // Reserve the grace period inside the deadline, not after it.
            const softTimer = setTimeout(() => terminate(new EngineFailure('timed_out', 'Simulation exceeded its execution time limit.')),
                Math.max(0, deadline - Date.now() - graceMs));
            const hardTimer = setTimeout(() => {
                failure = failure || new EngineFailure('timed_out', 'Simulation exceeded its execution time limit.');
                child.kill('SIGKILL');
            ...
            child.on('close', (code, exitSignal) => {
                ...
                const diagnostics: EngineDiagnostics = {exitCode: code, signal: exitSignal, stderr: Buffer.concat(stderr).toString('utf8'), launchError};
                if (failure) {
                ...
                if (code !== 0) {
                    ...
                    if (/version [`'](?:GLIBC|GLIBCXX|CXXABI)_[^\r\n]+not found/.test(diagnostics.stderr)) {
                        message = 'The server runtime is incompatible with the simulation engine. Run the backend using the Ubuntu 24.04 Docker image or a compatible Linux runtime (glibc 2.38 or newer).';
                    } else if (/error while loading shared libraries:/.test(diagnostics.stderr)) {
                        message = 'A system library required by the simulation engine is missing on the server. See server logs for details.';
                    }
                ...
                try {
                    ...
                    reject(new EngineFailure('failed', 'The simulation engine returned an invalid result. See server logs for details.', diagnostics));
                }
            });
        });
    } finally {
