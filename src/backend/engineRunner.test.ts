...
beforeEach(async () => {
    temporaryRoot = await mkdtemp(path.join(tmpdir(), 'atlantis-runner-test-'));
});
afterEach(async () => {
...
const runScript = (script: string, timeout = 1500, signal = new AbortController().signal, outputLimit = 4096) =>
    createEngineRunner(process.execPath, ['-e', script, '--'], outputLimit, temporaryRoot)(
        {attackers: {}, defenders: {}},
        2,
        signal,
        timeout,
...
it('force-kills a process that ignores termination within the execution budget and cleans its request', async () => {
    ...
    await expect(runScript("process.on('SIGTERM', () => {}); setInterval(() => {}, 10);", 600)).rejects.toMatchObject({
        status: 'timed_out',
    });
...
it('handles an unavailable engine', async () => {
    await expect(
        createEngineRunner('/does-not-exist', [], 4096, temporaryRoot)({}, 1, new AbortController().signal, 1000),
    ).rejects.toMatchObject({
        status: 'failed',
        message: expect.stringContaining('missing'),
        diagnostics: {launchError: expect.stringContaining('ENOENT')},
    });
});
...
it('identifies incompatible runtime libraries instead of blaming the battle setup', async () => {
    ...
    await expect(runScript(`console.error(${JSON.stringify(stderr)}); process.exitCode = 1;`)).rejects.toMatchObject({
        status: 'failed',
        message: expect.stringContaining('glibc 2.38'),
        diagnostics: {exitCode: 1, signal: null, stderr: stderr + '\n'},
    });
});

it('preserves bounded stderr and the exit code for diagnostics', async () => {
    await expect(
        runScript("process.stderr.write('x'.repeat(10000)); process.exitCode = 2;", 1500, undefined, 20000),
    ).rejects.toMatchObject({
        status: 'failed',
        message: expect.stringContaining('code 2'),
        diagnostics: {exitCode: 2, stderr: 'x'.repeat(8192)},
    });
});
...
it('identifies a missing system library', async () => {
    await expect(
        runScript(
            "console.error('engine: error while loading shared libraries: libexample.so: cannot open shared object file'); process.exitCode = 127;",
        ),
...
it('reports signal termination distinctly from an engine exit code', async () => {
    await expect(runScript("process.kill(process.pid, 'SIGTERM');")).rejects.toMatchObject({
        message: expect.stringContaining('SIGTERM'),
        diagnostics: {exitCode: null, signal: 'SIGTERM'},
    });
});
